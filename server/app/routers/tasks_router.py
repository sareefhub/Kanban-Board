...
router = APIRouter(prefix="/columns/{column_id}/tasks", tags=["tasks"], dependencies=[Depends(get_current_user)])
...
def get_tasks(column_id: int, current=Depends(get_current_user), db: Session = Depends(get_db)):
    tasks = db.query(Task).filter(Task.column_id == column_id).order_by(Task.position).all()
    return tasks

@router.get("/{task_id}", response_model=TaskOut)
def get_task(column_id: int, task_id: int, current=Depends(get_current_user), db: Session = Depends(get_db)):
